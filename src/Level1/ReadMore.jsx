...
export default function ReadMore() {
	...
	const filterResults = givenData =>{
		const filtered = givenData.filter((givenItem)=>{
			return (givenItem.id != data.item.id)
		})
		setSimData(filtered)
	}
	...
	useEffect(()=>{
		authAxios.get(`http://127.0.0.1:3000/similarArticles?id=${data.item.id}`)
		.then((res)=>{
  ...
  return (
	...
	<div className='container w-50 mt-4'>
		<span className={classes.tags}>{data.item.topic}</span>
		<h2 className={classes.title}>{data.item.title}</h2>
		...
		<hr />
		<h2 className='fw-bold'>More posts by Similar author</h2>
